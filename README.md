...
Warning: By default, the classes `kernel-filter-nonbuilt-cves.bbclass`, `kernel-generate-cve-exclusions.bbclass`, and `improve_kernel_cve_report.bbclass` require locating meta-vulnscout next to `poky` or `openembedded-core` folder to find the python script correctly.
This behaviour can be modified with the variable `SCRIPT_FOLDER`.
