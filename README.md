...
To install this meta-layer into your existing project, do so by adding it as a submodule. Add the repository into the `sources` directory and add it to your `build/conf/bblayers.conf` file using the following command:
...
-`vulnscout_improve_kernel_cve_report.bbclass` can be used to integrate the script `improve_kernel_cve_report.py` (reference: https://docs.yoctoproject.org/dev/singleindex.html#improve-kernel-cve-report-py).
...
This .bbclass is used by default and generates .rootfs.scouted.json files in the deploy directory (it can be removed with the variable `VULNSCOUT_KERNEL_IMPROVE_CVE`).
...
CQFD requires adding `docker-compose` (for Ubuntu 22.04) or `docker-compose-v2` (for Ubuntu 24.04) to your *.cfqd/docker/Dockerfile* and exporting the following variable:
